...
// TODO: do it better, with vagrant + ansible
//// 
...
[source, txt]
--
Feature: Words translation

  Scenario: Translate the word house
    Given The word house
    When I do the translation
    Then the result is dom
--
...
--
@Given("^The word (\\w+)$")
public void should_translate_the_word(Sting word) {
    get().uri(root, "translate/" + word);
}

@When("^I do the translation$")
public void execute_transation() {
    response = request.execute();
}

@Then("^the result is (.*)$")
public void validate_the_translation(String expectedWord) {
...
[source, groovy]
----
@Unroll("translation for #word is #translation")
def 'translate the words'() {
    expect:
		translation = get().uri(root, "translate/" + word).execute();
    where:
        words	| translation
        cinema 	| kino
        house  	| dom
